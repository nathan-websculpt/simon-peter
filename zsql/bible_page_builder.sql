...
CREATE OR REPLACE FUNCTION dynamic_page_builder_v_three(
    max_string_count integer,
    current_verse_id integer
  ) RETURNS setof json AS $$
DECLARE max_verse_id integer;

min_verse_id integer;

last_verse_of_page_one integer;
...
verse_string_length_count integer;

--total string.length, to be checked for being less than max_string_count
current_string_length integer;

--string.length of verse
BEGIN -- get last verse in table
...
last_verse_of_page_one := min_verse_id;

-- Immediately gets processed below
RAISE NOTICE 'max verse id: %',
max_verse_id;

RAISE NOTICE 'min verse id: %',
min_verse_id;
...
-- will start off as the min_verse_id (the first verse in table)
...
RAISE NOTICE 'last_verse_of_page_one: %',
last_verse_of_page_one;
...
IF current_verse_id <= last_verse_of_page_one THEN current_verse_id := min_verse_id;

last_id_of_page := min_verse_id;

ELSE last_verse_of_page_one := 0;
...
RAISE NOTICE 'Find the last verse ID based on the maximum string length ... last_id_of_page: %',
last_id_of_page;
...
RAISE NOTICE 'the next verse id will be first_id_of_next_page: %',
first_id_of_next_page;

-- Find the first verse ID for the previous page
IF current_verse_id > last_verse_of_page_one THEN verse_string_length_count := 0;

first_id_of_prev_page := current_verse_id;

--reset it to the current page, FirstVerseID; work backwords
RAISE NOTICE 'debugging... first_id_of_prev_page: %',
first_id_of_prev_page;

RAISE NOTICE 'debugging... verse_string_length_count: %',
verse_string_length_count;
...
RAISE NOTICE 'debugging, loop 1... first_id_of_prev_page: %',
first_id_of_prev_page;
...
RAISE NOTICE 'debugging, loop 2... first_id_of_prev_page: %',
first_id_of_prev_page;
...
ELSE first_id_of_prev_page := min_verse_id;

END IF;

RAISE NOTICE 'Right before building return, last_id_of_page: %',
last_id_of_page;

RAISE NOTICE 'Right before building return, first_id_of_prev_page: %',
first_id_of_prev_page;
